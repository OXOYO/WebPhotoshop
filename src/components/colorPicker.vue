<template>
  <div class="colorWrapper" v-show="showColorPicker">
    ...
    <div class="colorContent">
      <div class="colorMap">
        <h2>拖动圆圈选择颜色</h2>
        <div class="colorMapBox borderStyle"></div>
      </div>
      <div class="colorMapBar">
        <div class="colorBar borderStyle">
          <img src="../../src/assets/images/rangearrows.gif" :style="{top: topNum}">
        </div>
      </div>
        ...
        <div class="colorActiveSelect">
          <div v-for="(item, k) in [transleteArr[0], transleteArr[1]]" :key="item.id" :class="{marginStyle: k === 1}">
            <div class="colorActiveSelectList" v-for="(value, key) in item" :key="value.id">
              ...
              <input type="text" class="selectInput" :value="transleteArr[k][key]" @input="changeInput($event.target.value, k, key)">
...
export default {
  ...
  computed: {
    transleteArr () {
      return [{
        H: parseInt(this.colorArr[0].H),
        S: parseInt(this.colorArr[0].S * 100),
        V: parseInt(this.colorArr[0].V * 100)
      }, {
    ...
    topNum () {
      return 256 - (this.colorArr[0].H / 360) * 256 - 3.5 + 'px'
    },
    ...mapState([
      'globalColor',
      'showColorPicker'
  ...
  methods: {
    ...
    changeArr: function (hsv, rgb, col) {
      ...
      this.$store.commit('changeglobalColor', [rgb.R, rgb.G, rgb.B])
    },
